...
.. _accounting/get_started/localization:
...
Odoo's standard accounts, but they may have a different name and/or code. To handle these accounts,
simply edit the account name and/or account code of the standard Odoo accounts in your database to
use the name and code that you prefer. Other accounts may not have an analogous account in Odoo's
standard chart of accounts and can therefore be created manually or :ref:`imported
<essentials/export_import_data/import-data>`.

.. tip::
   Account codes must be unique per chart of accounts, so verify that any newly created accounts
   do not share the same code as an existing account before manually creating or importing the new
   accounts.
...
journal, and more are created by default, but additional journal can be created to manage different
...
that is purchasing them from you or selling them to you. They can also be configured to apply
automatically based on the customer's or vendor's location. Default fiscal positions are included
when installing your :doc:`fiscal localization package <../fiscal_localizations>`, but existing
fiscal positions can be adjusted and new fiscal positions can be created manually.
...
included when installing your :doc:`fiscal localization package <../fiscal_localizations>`, but
...
categories or create them manually. Product categories have multiple fields that relate to
...
By default, products adopt their :guilabel:`Income Account` and :guilabel:`Expense Account` from
their product category, but these can be overridden on the products themselves. Additionally,
products have other fields that relate to accounting, including:
...
.. tip::
   Import product categories first so that the product import can include which category each
   product belongs to.
...
via :ref:`SEPA direct debit <accounting/sepa_payments/sdd>`, you must have the customer's or
vendor's bank account set on their contact record.
...
<../../essentials/contacts>` or create them manually. Contacts have multiple fields that affect
their accounting records such as invoices and bills, including:
...
The following primary accounting configurations are crucial to successfully setting up your
accounting records. The majority of these are found in the Accounting settings
(:menuselection:`Accounting --> Configuration --> Settings`).

- :doc:`Company information <../../general/companies>` such as name, address, logo, phone, and email
  address
- :ref:`accounting/tax-returns/periodicity`
- :guilabel:`Rounding Method` - Whether the total tax amount should be rounded per line or per tax
  can be set in the :guilabel:`Taxes` section of the Accounting settings.
- :guilabel:`Prices` - Whether taxes should be :guilabel:`Tax Excluded` or :guilabel:`Tax Included`
  by default can be set in the :guilabel:`Taxes` section of the Accounting settings.
...
- :guilabel:`Default accounts` - Default accounts can be set in the following two sections of the
  ...
  - :guilabel:`Default accounts` - additional default accounts beyond those found on each journal
...
Companies that are transitioning to Odoo with existing account balances, open invoices and bills,
and an opening inventory count need to enter that information in their Odoo database so that they
...
.. example::
   A company has a current balance of $1000 in their account receivable, reflecting the $1000 that
   they are owed for their open invoices.

   If these open invoices and the balance of the account receivable are both created in Odoo without
   making any changes, the account receivable will mistakenly reflect a balance of $2000:

   - $1000 from the opening trial balance of the account receivable
   - $1000 from the creation of the open invoices in Odoo

The same concept applies to open vendor bills and the account payable.

To avoid this error, using *clearing accounts*, which are accounts that temporarily hold the value
...
Determine when to transition your accounting workflows to Odoo. There are three options for when to
transition:

- At the end of the fiscal year: this is the easiest option because it minimizes the data that needs
  to be transferred.
- At the end of the tax period: with this option, you can choose to post all of the past entries
  from the past entries from the period or only the balance of each account at the end of the
  period.
- In the middle of a tax period: this option is not recommended because it causes additional work
  ...
  filing process since the data of the two systems must be combined.
...
.. note::
   ...
   start with a clean general ledger. Additionally, creating an opening bank transaction(s) is
   necessary to match the current bank account(s) balance.
...
.. warning::
   Before starting the process of creating your opening entries in Odoo, verify that your data meets
...
Companies transitioning to Odoo need to import or manually create their open (meaning existing but
not yet fully paid) invoices and vendor bills (as well as any credit notes ore refunds) in Odoo so
that no invoice or bill is lost in the transition. Importing these records is largely the same
process for both invoices and bills and is done in three steps:
...
.. note::
   ...
   due instead of with individual invoice/bill lines. This avoids common errors such as tax accounts
...
- For invoices and credit notes, go to :menuselection:`Accounting --> Customers --> Invoices`, click
  the :icon:`fa-cog` (:guilabel:`gear`) icon, and click :icon:`fa-download` :guilabel:`Import
  records`. Click :icon:`fa-download` :guilabel:`Import Template for Invoices` to download the
  template.
- For vendor bills and refunds, go to :menuselection:`Accounting --> Vendors --> Bills`, click the
  :icon:`fa-cog` (:guilabel:`gear`) icon, and click :icon:`fa-download` :guilabel:`Import records`.
  Click :icon:`fa-download` :guilabel:`Import Template for Bills` to download the template.
...
.. list-table::
   ...
   * - Due Date
     - Date the invoice/bill is due - each invoice can have either a due date or payment terms, but
       not both.
   * - Payment Terms
     - The :doc:`payment terms <customer_invoices/payment_terms>` of the invoice/bill each invoice
       can have either a due date or payment terms, but not both. Payment terms are only recommended
       if using payment installments.
   * - Journal
...
- Check for negative invoices and bills:
  ...
  - If any invoices are for negative amounts, they are credit notes and should be imported
    separately as a positive value under the :menuselection:`Customers --> Credit Notes` menu.
    Change the :guilabel:`Number` from `INV/YEAR/NUMBER` to `RINV/YEAR/NUMBER`, and (optionally)
    change the :guilabel:`Reference` to `Reversal of: [INV Number]`.
...
:ref:`Import <essentials/export_import_data/import-data>` the fields prepared in the
:ref:`preparation <accounting/get_started/prep-bills>` section:

#. Go to the appropriate menu:

   - For customer invoices, go to :menuselection:`Customers --> Invoices`.
   - For vendor bills, go to :menuselection:`Vendors --> Bills`.
   - For credit notes, go to :menuselection:`Customers --> Credit Notes`.
   - For vendor refunds, go to :menuselection:`Vendors --> Refunds`.

#. Click the :icon:`fa-cog` (:guilabel:`Actions`), then click :icon:`fa-download`
   :guilabel:`Import records`.
#. Click the :guilabel:`Upload Data File` button, select the appropriate file, and click
   :guilabel:`Select`.
#. Verify that the correct fields are selected in the :guilabel:`Odoo Field` column.
#. Click :guilabel:`Test` and, when the :guilabel:`Everything seems valid` banner appears, click
   :guilabel:`Import`.
#. The records are all imported as drafts. From the resulting list view, select the imported draft
   records, click :icon:`fa-gear` :guilabel:`Actions`, and click :guilabel:`Confirm Entries`.
#. Repeat the process for the other record types (invoices, bills, credit notes, and refunds).
...
.. tip::
   ...
   cost. Repeat the process as any times as needed, once for each different cost.
...
:icon:`fa-gear` (:guilabel:`gear`) icon, and click :icon:`fa-download` :guilabel:`Import records`.
Click :icon:`fa-download` :guilabel:`Import Template for Inventory Adjustments` to download the
template.
...
:ref:`Import <essentials/export_import_data/import-data>` the fields prepared in the
:ref:`preparation <accounting/get_started/prep-inventory>` section:
...
(:guilabel:`gear`) icon, and click :icon:`fa-download` :guilabel:`Import records`. Click
:icon:`fa-download` :guilabel:`Import Template for Misc. Operations` to download the template.
...
.. list-table::
   ...
   * - Journal Items/Debit
     - The debit value of the account in the company's main currency as a positive number. If the
       line has a credit, this field should be left blank. Do not enter `0.00` or a negative value.
...
.. important::
   ...
   - Each line should have only one value in either the :guilabel:`Journal Items/Debit` or
     :guilabel:`Journal Items/Credit` column, and the other column of the line should remain blank.
     Do not enter `0.00` or a negative value in any column.
...
Validate opening trial balance
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
:ref:`Import <essentials/export_import_data/import-data>` the fields prepared in the
:ref:`preparation <accounting/get_started/prep-tb>` section:
